...
def load_model() -> torch.nn.Module:
    ...
    state = torch.load(MODE_PATH_SAFE(MODEL_PATH), map_location=DEVICE)
    model.load_state_dict(state)
    model.eval()
    return model


def MODE_PATH_SAFE(path: Path):
    # Permite passar string ou Path sem erro
    return str(path)
...
def compute_class_stats(mask: np.ndarray):
    """
    Calcula quantidade de pixels e percentual por classe.
